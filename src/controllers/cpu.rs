...
pub async fn cpu(_: &mut System) -> CpuInfo {
    ...
    CpuInfo {
        ...
        current_voltage: Some(
            windoww
                .0
                .current_voltage as f32
                / 1000.0,
        ),
        ...
        load_percentage: Some(
            windoww
                .0
                .load_percentage as f32
                / 100.0,
        ),
    }
}
...
pub fn cpu_threads(system: &mut System) -> Vec<(String, Color)> {
    ...
    for (i, usage) in cpu_usages
        .iter()
        .enumerate()
    {
        let color = Color::Rgb(rng.random(), rng.random(), rng.random());
        lines.push((format!("Nucleo Logico {}: {:.1}%", i + 1, usage * 100.0), color));
    }
